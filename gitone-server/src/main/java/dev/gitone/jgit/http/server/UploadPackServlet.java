...
import dev.gitone.jgit.http.server.UploadPackErrorHandler.UploadPackRunnable;
...
import static dev.gitone.jgit.http.server.ServletUtils.*;
import static dev.gitone.jgit.http.server.UploadPackErrorHandler.statusCodeForThrowable;
...
class UploadPackServlet extends HttpServlet {
	...
	static class InfoRefs extends SmartServiceInfoRefs {
		...
		InfoRefs(UploadPackFactory<HttpServletRequest> uploadPackFactory,
				List<Filter> filters) {
			super(GitSmartHttpTools.UPLOAD_PACK, filters);
	...
	public void doPost(HttpServletRequest req, HttpServletResponse rsp)
			throws IOException {
		if (!GitSmartHttpTools.UPLOAD_PACK_REQUEST_TYPE.equals(req.getContentType())) {
	...
	private void upload(HttpServletRequest req, HttpServletResponse rsp)
		...
		try (UploadPack up = (UploadPack) req.getAttribute(ATTRIBUTE_HANDLER)) {
			up.setBiDirectionalPipe(false);
			rsp.setContentType(GitSmartHttpTools.UPLOAD_PACK_RESULT_TYPE);
	...
	private void defaultUploadPackHandler(HttpServletRequest req,
		...
		try {
			...
			if (!rsp.isCommitted()) {
				...
				GitSmartHttpTools.sendError(req, rsp, statusCodeForThrowable(e), msg);
			}
		}
	}
