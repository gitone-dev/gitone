...
@Controller
public class ProjectPolicy extends NamespacePolicy {

    private static final Map<Access, Set<Action>> accessActionsMap = new HashMap<>();

    static {
        for (Access value : Access.values()) {
            accessActionsMap.put(value, ProjectPolicy.forAccess(value));
        }
    }
    ...
    @Autowired
    public ProjectPolicy(MemberDao memberDao) {
        super(memberDao);
    }

    @Override
    public Map<Access, Set<Action>> accessActions() {
        return ProjectPolicy.accessActionsMap;
    }
    ...
    public MemberEntity assertPermission(NamespaceEntity namespaceEntity, Action action) {
        ...
        if (isAuthenticated()) {
            if (namespaceEntity.getId().equals(namespaceId())) {
                return null;
            }
