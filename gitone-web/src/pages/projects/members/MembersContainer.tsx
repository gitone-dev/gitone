...
function MembersContainer(props: Props) {
  ...
  const { data, loading, error, fetchMore } = useProjectMembersQuery({
  ...
  const namespaceId = data?.project.id;
  const edges = data?.project?.members?.edges;
  const pageInfo = data?.project?.members?.pageInfo;
