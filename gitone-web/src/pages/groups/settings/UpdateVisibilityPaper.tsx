...
function UpdateVisibilityPaper(props: Props) {
  ...
  const [visibility] = useState(group.visibility);
  const { handleSubmit, register } = useForm<UpdateGroupVisibilityInput>();
  const [updateGroupVisibilityMutation] = useUpdateGroupVisibilityMutation();

  const onSubmit = handleSubmit((input: UpdateGroupVisibilityInput) => {
    updateGroupVisibilityMutation({
      variables: { input },
      onCompleted() {
